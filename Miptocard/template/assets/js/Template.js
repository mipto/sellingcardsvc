var Template = {
	...
	_initAjaxPagesLoading: function(el, selector){
		...
		$(selector, el).click(function(event){
			...
			// if link element has target attribute or data-disableajax, prevent links processing with ajax
			if (jEl.attr('target') || jEl.data('disableajax')) {
				return;
			}
		...
		$.ajax(url, {
			success:function(responseText){
				...
				if (newDomActiveItem.length) {	
					...
					if(newDomActiveItem.attr('href') == "index.html"){
						console.log("INDEX", newDomActiveItem.attr('href'));
